...
### 3. 스프링 원리 이해 - 객체지향원리 적용

### 4. 스프링컨테이너, 빈

- 스프링 컨테이너 생성 -> 스프링 빈 저장소에 빈 생성 -> 스프링 빈 등록(의존관계 설정)

- 빈 조회 

    - 빈 이름으로 조회, 타입으로 조회, 구체타입으로 조회
...
- 객체 인스턴스를 공유하기 때문에 무상태로 설계한다.(stateless)


### 6. 컴포넌트스캔

- `@ComponentScan`은 `@Component`의 모든 클래스를 스프링 빈으로 등록한다. 

- `basePackages` 패키지를 포함하여 하위 패키지 모두 탐색한다.
